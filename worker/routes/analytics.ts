...
analyticsRoutes.get("/payment-tracking", async (c) => {
  ...
  // "Full payment received" is derived from payment_status = 'completed'
  ...
  const fullPayment = await c.env.DB.prepare(
    `SELECT SUM(CASE WHEN LOWER(TRIM(COALESCE(ci.value,''))) = 'completed' THEN 1 ELSE 0 END) AS received,
            COUNT(*) AS total
     FROM events e
