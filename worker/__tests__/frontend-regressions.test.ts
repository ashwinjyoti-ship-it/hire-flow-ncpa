...
describe("frontend regression guards", () => {
  ...
  it("applies optimistic checklist updates via shared hook instead of full event refetch", () => {
    ...
    expect(hook).toContain("patchEventDetailCache");
    expect(hook).not.toContain("await qc.cancelQueries");
    ...
    expect(detail).toContain("FieldSavingSpinner");
  ...
  it("keeps MoM-related event form fields for theatre canteen timings and officer contact", () => {
    ...
    expect(pocFields).toContain("event_company_name");
  ...
  it("exposes Generate MoM on the event lifecycle panel", () => {
    ...
    expect(detail).toContain("mom-rich-preview");
  });
